...
\begin{frame}{Aim}
  \begin{itemize}
  \item Concepts: reproducible workflows and automation
  \item Tech: files, \$PATH, YAML, apptainer, conda, Snakemake, Omnibenchmark
  \end{itemize}
\end{frame}
...
\begin{frame}{Text files vs. executable files}
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item Platform-specific (e.g., Linux amd64)
...
\end{lstlisting}
    ...
    \begin{itemize}
        ...
        \item Software (executables!) "lives" in directories listed in \texttt{\$PATH}
        \item Can be modified temporarily or permanently
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{Understanding where executables live}
    \begin{lstlisting}[language=bash]
...
\end{lstlisting}
    ...
    \vspace{0.3cm}
    This is crucial for understanding conda and apptainer
\end{frame}
...
\begin{frame}[fragile]{YAML basics: Indentation Rules}
    \textbf{Critical Rule:} YAML uses \textbf{spaces for indentation}, avoid tabs
...
\begin{frame}{What is Conda?}
    \textbf{Conda} is a package and environment (e.g., PATH) management system
...
\end{lstlisting}
    ...
    \textbf{This means:}
    \begin{itemize}
        \item Software in the active environment is found first (PATH)
        \item Each environment has its own \texttt{bin/} directory
        \item Environments are isolated - no conflicts (they claim)
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{Conda environment YAML files}
...
\begin{frame}{What is Singularity? (and Apptainer)?}
    ...
    \begin{itemize}
        \item No root daemon required, so HPC-friendly
        \item Can run Docker images
          \item Singularity doesn't run on MacOS; Docker does
    \end{itemize}
\end{frame}
...
#  (singularity-native registries also exist)
...
\begin{frame}[fragile]{Singularity and \$PATH}
    Containers have their own \$PATH
...
\begin{frame}{Components of reproducible workflows}
    \begin{enumerate}
...
\begin{frame}{What is Omnibenchmark?}
    ...
    \textbf{Key Features:}
    \begin{itemize}
        \item Formal description of benchmark design and components
        \item Automated workflow execution
        \item Benchmarking-inspired features (profiling, visualization, attribution, etc)
...
\begin{frame}{Summary}
    \begin{itemize}
        ...
        \item \textbf{PATH}: Defines which executables are found, and when
        ...
        \item \textbf{Omnibenchmark}: Integrates these to ease benchmarking (they claim)
    \end{itemize}
