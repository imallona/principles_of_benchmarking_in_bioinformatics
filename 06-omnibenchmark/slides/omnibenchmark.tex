...
# 1. Install omnibenchmark (mind you can pin specific versions, e.g. pip install omnibenchmark==0.3.0 instead)
...
## ob >= 0.4.0
ob run Clustering.yaml --cores 2

# Options:
# -b, --benchmark: Path to configuration YAML
# --local-storage: Store results locally; default in ob >= 0.4.0
# --cores: Number of parallel processes
# --dry-run: # use --dry instead on ob >= 0.4.0
...
\begin{frame}{Software environments / backends}
    ...
    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{Conda} (default): conda nondefaults + bioconda recipes
            \begin{itemize}
                \item Config: \texttt{Clustering\_conda.yaml}
                \item Best for: Local development
            \end{itemize}
...
\begin{frame}{Contributing new modules}
    ...
    \vspace{0.3cm}
    \textbf{Tip:} For ob $\ge$ 0.4.0 use \texttt{ob create module} to scaffold the repository
...
\begin{frame}{Summary}
    ...
    \begin{enumerate}
        ...
        \item Clone example: \texttt{git clone https://github.com/omnibenchmark/clustering\_example}
        \item Try a dry-run: \texttt{ob run benchmark -b Clustering.yaml --dry-run} (ob < 0.4.0) or \texttt{ob run -b Clustering.yaml --dry} (ob $\ge$ 0.4.0)
    \end{enumerate}
\end{frame}
