...
\begin{frame}{What is Omnibenchmark?}
    ...
    \vspace{0.3cm}
    Website: \texttt{https://omnibenchmark.org}
    
    Docs: \texttt{https://docs.omnibenchmark.org}
\end{frame}
...
\begin{frame}{Omnibenchmarks architecture}
...
\begin{frame}{The clustering example}
    \textbf{Repository:} \texttt{github.com/omnibenchmark/clustering\_example}
...
\begin{frame}{Clustbench attribution}
    ...
    \vspace{0.3cm}
    Data repository: \texttt{github.com/imallona/clustbench\_data}
\end{frame}
...
\texttt{https://docs.omnibenchmark.org/latest/howto/\#install-omnibenchmark}
\end{frame}
...
\begin{lstlisting}[language=bash]
# Basic run (6 cores, local storage)
ob run benchmark -b Clustering.yaml --local-storage --cores 6

# Options:
# -b, --benchmark: Path to configuration YAML
# --local-storage: Store results locally (not on Renku)
# --cores: Number of parallel processes
# --dry-run: Show what would be executed without running

# For larger runs (recommended 30+ cores)
ob run benchmark -b Clustering.yaml --local-storage --cores 30

# Expected runtime:
# - 6 cores: ~8 hours for 97% completion
# - 30 cores: ~4 hours for 50% completion
\end{lstlisting}
...
\begin{frame}{Data modules}
    \textbf{Data repository:} \texttt{github.com/imallona/clustbench\_data}
...
\begin{frame}{Method modules}
    ...
    \vspace{0.3cm}
    \begin{enumerate}
        \item \textbf{fastcluster} (\texttt{github.com/imallona/clustbench\_fastcluster})
            \begin{itemize}
                \item Linkages: complete, ward, average, weighted, median, centroid
            \end{itemize}
        
        \item \textbf{sklearn} (\texttt{github.com/imallona/clustbench\_sklearn})
            \begin{itemize}
                \item Methods: birch, kmeans, spectral, Gaussian mixture
            \end{itemize}
        
        \item \textbf{genieclust} (\texttt{github.com/imallona/clustbench\_genieclust})
            \begin{itemize}
                \item Methods: genie, GIC, ICA with various thresholds
            \end{itemize}
        
        \item \textbf{FCPS} (\texttt{github.com/imallona/clustbench\_fcps})
            \begin{itemize}
                \item Methods: Minimax, MinEnergy, HDBSCAN, Diana, Fanny, etc.
            \end{itemize}
        
        \item \textbf{agglomerative} (\texttt{github.com/imallona/clustbench\_agglomerative})
            \begin{itemize}
...
# Check Snakemake output
# Shows completed/remaining jobs
...
\begin{frame}{Interpreting Results}
    ...
    \begin{itemize}
        \item Which methods perform best on average?
        ...
        \item Are results statistically significant?
    \end{itemize}
    ...
    \textbf{Remember:}
    \begin{itemize}
        \item No single "best" method for all cases
        \item Consider multiple metrics
        \item Context matters (real vs. synthetic data)
    \end{itemize}
\end{frame}

\begin{frame}{Contributing New Modules}
    \textbf{Want to add your method or dataset?}
    ...
    \vspace{0.3cm}
    \textit{Community contributions are encouraged!}
\end{frame}

\begin{frame}[fragile]{Creating a Method Module}
\begin{lstlisting}[language=python]
# Minimal method module structure
# my_clustering_method/src/run.py

import argparse
import pandas as pd
from my_method import cluster
...
def main():
    ...
    # Run clustering
    labels = cluster(data, n_clusters=args.n_clusters)
...
\begin{frame}{Best Practices for Modules}
    ...
    \begin{itemize}
        ...
        \item Modify input data files
        \item Ignore errors silently
        \item Use undocumented dependencies
        \item Skip validation of inputs
    \end{itemize}
\end{frame}
...
\begin{frame}{Advantages of Omnibenchmark}
    \textbf{For benchmark creators:}
    \begin{itemize}
        \item Automated orchestration
        \item Built-in provenance tracking
        \item Community contributions
        \item Continuous updates
    \end{itemize}
    
    \vspace{0.3cm}
    \textbf{For method developers:}
    \begin{itemize}
        \item Fair comparison to existing methods
        \item No need to reimplement baselines
        \item Standardized evaluation protocol
        \item Continuous testing on new data
    \end{itemize}
    
    \vspace{0.3cm}
    \textbf{For users:}
    \begin{itemize}
        \item Up-to-date comparisons
        \item Reproducible results
        \item Easy to explore tradeoffs
...
\begin{frame}{Limitations and Considerations}
    \textbf{Be aware of:}
    \begin{itemize}
        \item \textbf{Computational cost}: Full benchmarks can take hours/days
        \item \textbf{Architecture requirements}: amd64 only (no arm64)
        \item \textbf{Storage needs}: Results can be large
        \item \textbf{Complexity}: Steep learning curve for first-time users
        \item \textbf{Maintenance}: Modules need updates as software evolves
    \end{itemize}
    
    \vspace{0.3cm}
    \textbf{Not suitable for:}
    \begin{itemize}
        \item Quick one-off comparisons (use simpler tools)
        \item Methods requiring interactive tuning
        \item Proprietary methods that can't be shared
    \end{itemize}
    
    \vspace{0.3cm}
    \textbf{Best for:}
    \begin{itemize}
        \item Large-scale, ongoing benchmarks
        \item Community-driven comparisons
...
\begin{frame}{Summary}
    ...
    \begin{itemize}
        ...
        \item Best for large-scale, ongoing benchmarks
    \end{itemize}
    ...
    \begin{enumerate}
        ...
        \item Run small test (if resources allow)
    \end{enumerate}
\end{frame}
...
\begin{frame}{Resources}
    ...
    \begin{itemize}
        ...
        \item See Omnibenchmark documentation for platform papers
    \end{itemize}
\end{frame}
